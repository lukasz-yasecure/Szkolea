...
try
{
    ...
    $st = $tm->getSearchTemplate($sys, $bar->getContent());
    // need results tmp

    $cm = new CategoryManager();
    $ud = new UserData();
    $c = $cm->getCategories($dbc, $ud->getIdForLeftMenu());
    $lmlt = $tm->getLeftMenuListTemplate($sys, $c, $ud->getWhatForLeftMenu());
    $lmt = $tm->getLeftMenuTemplate($sys, $cm, $lmlt);

    $s = $ud->getSearch();
    $rlt = $tm->getResultsListTemplate($sys);
    $rt = $tm->getResultsTemplate($sys, $rlt);

    $it = $tm->getIndexTemplate($sys, $st->getContent(), $lmt->getContent(), $rt->getContent());
    $mt = $tm->getMainTemplate($sys, $it->getContent(), BFEC::showAll());
    echo $mt->getContent();
}
