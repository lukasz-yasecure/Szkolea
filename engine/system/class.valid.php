...
class Valid {
    ...
    static function serv_name($s) {
        ...
        if (preg_match("/^[a-ząęóćłńśźż \-\!\?\"\'\,\.\;\d]+$/iu", $s))
            return true;
        else
            return false;
    }
    ...
    static function program($l, $s) {
        if ($l == 0) {
            ...
            $t = explode('_', $s);
            if (!is_numeric($t[0]) || !is_numeric($t[1]) || !is_numeric($t[2]))
                return false;
        }
        else if ($l == 3) {
            $t = explode('_', $s);
            if (!is_numeric($t[0]) || !is_numeric($t[1]) || !is_numeric($t[2]) || !is_numeric($t[3]))
                return false;
            else {
    ...
    static function id($s) {
        ...
        if ($st == 0)
            return Valid::kategoria($s);
        else if ($st == 1)
            return Valid::obszar($s);
        else if ($st == 2)
            return Valid::tematyka($s);
        else if ($st == 3)
            return Valid::modul($s);
    ...
    static function text($s) {
        ...
        if (preg_match("/^[a-ząęóćłńśźż \-\!\?\"\'\,\.\;\:\d]+$/ium", $s))
            return true;
        else
            return false;
    }
    ...
    static function name($s) {
        ...
        if (preg_match("/^[a-ząęóćłńśźż]+$/iu", $s))
            return true;
        else
            return false;
    }
    ...
    static function surname($s) {
        ...
        if (preg_match("/^[a-ząęóćłńśźż]+\-{0,1}[a-ząęóćłńśźż]*$/iu", $s))
            return true;
        else
            return false;
    }
    ...
    static function house_number($s) {
        ...
        if (preg_match("/^[a-z]*\d+[a-z]*(|\/\d+[a-z]*)$/iu", $s))
            return true;
        else
            return false;
    }
    ...
    static function add_comm_days_continuity($days, $long) {
        if (in_array(0, $days) || is_null($long))
            return true;
    ...
    static function add_comm_date_long($d1, $d2, $l) {
        if (is_null($d1) || is_null($d2) || is_null($l))
            return false;
