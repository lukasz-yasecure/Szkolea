...
FROM commisions C INNER JOIN users_324 U ON C.id_user = U.id_user INNER JOIN commisions_group CG ON CG.id_comm = C.id_comm INNER JOIN users_324 U2 ON U2.id_user= CG.id_user GROUP BY CG.id_comm, CG.id_user';
    ...
    public static function setPackageForUser($id_user, $pakiet) {
        ...
        $sql = 'INSERT INTO `users_packages` (`id_user`, `id_pakietu`, `uslugi`, `oferty`, `date_begin`, `date_end`) VALUES (' . $id_user . ', ' . $pakiet['id_pakietu'] . ', ' . $pakiet['uslugi'] . ', ' . $pakiet['oferty'] . ', ' . time() . ', ' . ( time() + $pakiet['waznosc'] * 86400 ) . ')';
        return $sql;
    }
    ...
    public static function getDataProfilePaidInvoiceList($id_user) {
        return 'SELECT * FROM `faktury` WHERE `id_user`=' . $id_user . ' AND `numer_fv` IS NOT NULL'; // pobierane faktury VAT
    }

    public static function getDataProfileInvoice($id_faktura) {
