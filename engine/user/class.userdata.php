...
class UserData {
    ...
    public function getRegisterFormData() {
        ...
        if (!isset($_POST['regulamin']) || $_POST['regulamin'] != 1)
            BFEC::add('wymagana jest akceptacja regulaminu!');

        if (!isset($_POST['ukind']) || empty($_POST['ukind']) || ($_POST['ukind'] != 'K' && $_POST['ukind'] != 'D'))
            BFEC::add('musisz okreslic rodzaj konta');
        else
            RFD::add('regForm', 'ukind', $_POST['ukind']);

        if (!isset($_POST['email']) || empty($_POST['email']) || !Valid::email($_POST['email']))
            BFEC::add('musisz podac poprawny email');
        else
            RFD::add('regForm', 'email', $_POST['email']);

        if (!isset($_POST['pass1']) || !isset($_POST['pass2']) || empty($_POST['pass1']) || empty($_POST['pass2']))
            BFEC::add('musisz dwukrotnie podac haslo');
        else {
            if (!Valid::password($_POST['pass1']))
                BFEC::add('hasło nie spełnia norm');
            else {
                if ($_POST['pass1'] !== $_POST['pass2'])
                    BFEC::add('hasła nie pasuja');
            }
        }
        ...
        if (!isset($_POST['os_name']) || empty($_POST['os_name']))
            $os++;
        else {
            if (Valid::name($_POST['os_name']))
                RFD::add('regForm', 'os_name', $_POST['os_name']);
            else {
                BFEC::add('imie moze skladac sie tylko i wylacznie z liter');
                $os++;
            }
        }
        if (!isset($_POST['os_surname']) || empty($_POST['os_surname']))
            $os++;
        else {
            if (Valid::surname($_POST['os_surname']))
                RFD::add('regForm', 'os_surname', $_POST['os_surname']);
            else {
                BFEC::add('w nazwisku wystepuja niedozwolone znaki');
                $os++;
            }
        }
        if (!isset($_POST['os_street']) || empty($_POST['os_street']))
            $os++;
        else {
            if (Valid::street($_POST['os_street']))
                RFD::add('regForm', 'os_street', $_POST['os_street']);
            else {
                BFEC::add('w nazwie ulicy wystepuja niedozwolone znaki');
                $os++;
            }
        }
        if (!isset($_POST['os_house_number']) || empty($_POST['os_house_number']))
            $os++;
        else {
            if (Valid::house_number($_POST['os_house_number']))
                RFD::add('regForm', 'os_house_number', $_POST['os_house_number']);
            else {
                BFEC::add('w numerze domu/mieszkania wystepuja niedozwolone znaki');
                $os++;
            }
        }
        if (!isset($_POST['os_postcode']) || empty($_POST['os_postcode']))
            $os++;
        else {
            if (Valid::postcode($_POST['os_postcode']))
                RFD::add('regForm', 'os_postcode', $_POST['os_postcode']);
            else {
                BFEC::add('w kodzie pocztowym wystepuja niedozwolone znaki');
                $os++;
            }
        }
        if (!isset($_POST['os_city']) || empty($_POST['os_city']))
            $os++;
        else {
            if (Valid::city($_POST['os_city']))
                RFD::add('regForm', 'os_city', $_POST['os_city']);
            else {
                BFEC::add('w miescie wystepuja niedozwolone znaki');
                $os++;
            }
        }
        if (!isset($_POST['os_phone']) || empty($_POST['os_phone']))
            $os++;
        else {
            if (Valid::phone($_POST['os_phone']))
                RFD::add('regForm', 'os_phone', $_POST['os_phone']);
            else {
        ...
        if ($os > 0 && $os < 7)
            BFEC::add('nalezy podac wszystkie wymagane dane osobowe');
        ...
        if (!isset($_POST['f_name']) || empty($_POST['f_name']))
            ...
            if (Valid::name($_POST['f_name']))
                RFD::add('regForm', 'f_name', $_POST['f_name']);
            else {
                BFEC::add('imie moze skladac sie tylko i wylacznie z liter');
                $f++;
            }
        }
        if (!isset($_POST['f_surname']) || empty($_POST['f_surname']))
            $f++;
        else {
            if (Valid::surname($_POST['f_surname']))
                RFD::add('regForm', 'f_surname', $_POST['f_surname']);
            else {
                BFEC::add('w nazwisku wystepuja niedozwolone znaki');
                $f++;
            }
        }
        if (!isset($_POST['f_position']) || empty($_POST['f_position']))
            $f++;
        else {
            if (Valid::position($_POST['f_position']))
                RFD::add('regForm', 'f_position', $_POST['f_position']);
            else {
        ...
        if (!isset($_POST['f_company']) || empty($_POST['f_company']))
            ...
            if (Valid::company($_POST['f_company']))
                RFD::add('regForm', 'f_company', $_POST['f_company']);
            else {
                BFEC::add('w nazwie firmy wystepuja niedozwolone znaki');
                $f++;
            }
        }
        if (!isset($_POST['f_street']) || empty($_POST['f_street']))
            $f++;
        else {
            if (Valid::street($_POST['f_street']))
                RFD::add('regForm', 'f_street', $_POST['f_street']);
            else {
                BFEC::add('w nazwie ulicy wystepuja niedozwolone znaki');
                $f++;
            }
        }
        if (!isset($_POST['f_house_number']) || empty($_POST['f_house_number']))
            $f++;
        else {
            if (Valid::house_number($_POST['f_house_number']))
                RFD::add('regForm', 'f_house_number', $_POST['f_house_number']);
            else {
                BFEC::add('w numerze domu/mieszkania wystepuja niedozwolone znaki');
                $f++;
            }
        }
        if (!isset($_POST['f_postcode']) || empty($_POST['f_postcode']))
            $f++;
        else {
            if (Valid::postcode($_POST['f_postcode']))
                RFD::add('regForm', 'f_postcode', $_POST['f_postcode']);
            else {
                BFEC::add('w kodzie pocztowym wystepuja niedozwolone znaki');
                $f++;
            }
        }
        if (!isset($_POST['f_city']) || empty($_POST['f_city']))
            $f++;
        else {
            if (Valid::city($_POST['f_city']))
                RFD::add('regForm', 'f_city', $_POST['f_city']);
            else {
                BFEC::add('w miescie wystepuja niedozwolone znaki');
                $f++;
            }
        ...
        if (!isset($_POST['f_phone']) || empty($_POST['f_phone']))
            ...
            if (Valid::phone($_POST['f_phone']))
                RFD::add('regForm', 'f_phone', $_POST['f_phone']);
            else {
                BFEC::add('telefon w zlym formacie');
                $f++;
            }
        }
        if (!isset($_POST['f_nip']) || empty($_POST['f_nip']))
            $f++;
        else {
            if (Valid::nip($_POST['f_nip']))
                RFD::add('regForm', 'f_nip', $_POST['f_nip']);
            else {
        ...
        if ($f > 0 && $f < 10)
            BFEC::add('nalezy podac wszystkie wymagane dane firmowe');

        if ($os == 7 && $f == 10)
            BFEC::add('musisz uzupelnic wszystkie wymagane dane osobowe lub firmowe');
        ...
        if (isset($_POST['f_regon']) && !empty($_POST['f_regon'])) {
            if (Valid::regon($_POST['f_regon'])) {
                ...
                BFEC::add('regon w zlym formacie');
        }
        ...
        if (BFEC::isError())
            throw new ErrorsInRegisterForm('');
    ...
    public function getProfileEditFormData() {
        ...
        if (!isset($_POST['os_name']) || empty($_POST['os_name']))
            ...
            if (Valid::name($_POST['os_name']))
                RFD::add('profEditForm', 'os_name', $_POST['os_name']);
            else {
                BFEC::add('imie moze skladac sie tylko i wylacznie z liter');
                $os++;
            }
        ...
        if (!isset($_POST['os_surname']) || empty($_POST['os_surname']))
            ...
            if (Valid::surname($_POST['os_surname']))
                RFD::add('profEditForm', 'os_surname', $_POST['os_surname']);
            else {
                BFEC::add('w nazwisku wystepuja niedozwolone znaki');
                $os++;
            }
        ...
        if (!isset($_POST['os_street']) || empty($_POST['os_street']))
            ...
            if (Valid::street($_POST['os_street']))
                RFD::add('profEditForm', 'os_street', $_POST['os_street']);
            else {
                BFEC::add('w nazwie ulicy wystepuja niedozwolone znaki');
                $os++;
            }
        ...
        if (!isset($_POST['os_house_number']) || empty($_POST['os_house_number']))
            ...
            if (Valid::house_number($_POST['os_house_number']))
                RFD::add('profEditForm', 'os_house_number', $_POST['os_house_number']);
            else {
                BFEC::add('w numerze domu/mieszkania wystepuja niedozwolone znaki');
                $os++;
            }
        ...
        if (!isset($_POST['os_postcode']) || empty($_POST['os_postcode']))
            ...
            if (Valid::postcode($_POST['os_postcode']))
                RFD::add('profEditForm', 'os_postcode', $_POST['os_postcode']);
            else {
                BFEC::add('w kodzie pocztowym wystepuja niedozwolone znaki');
                $os++;
            }
        ...
        if (!isset($_POST['os_city']) || empty($_POST['os_city']))
            ...
            if (Valid::city($_POST['os_city']))
                RFD::add('profEditForm', 'os_city', $_POST['os_city']);
            else {
                BFEC::add('w miescie wystepuja niedozwolone znaki');
                $os++;
            }
        ...
        if (!isset($_POST['os_phone']) || empty($_POST['os_phone']))
            ...
            if (Valid::phone($_POST['os_phone']))
                RFD::add('profEditForm', 'os_phone', $_POST['os_phone']);
            else {
                BFEC::add('telefon w zlym formacie');
                $os++;
            }
        ...
        if (!isset($_POST['f_name']) || empty($_POST['f_name']))
            ...
            if (Valid::name($_POST['f_name']))
                RFD::add('profEditForm', 'f_name', $_POST['f_name']);
            else {
                BFEC::add('imie moze skladac sie tylko i wylacznie z liter');
                $f++;
            }
        ...
        if (!isset($_POST['f_surname']) || empty($_POST['f_surname']))
            ...
            if (Valid::surname($_POST['f_surname']))
                RFD::add('profEditForm', 'f_surname', $_POST['f_surname']);
            else {
                BFEC::add('w nazwisku wystepuja niedozwolone znaki');
                $f++;
            }
        ...
        if (!isset($_POST['f_position']) || empty($_POST['f_position']))
            ...
            if (Valid::position($_POST['f_position']))
                RFD::add('profEditForm', 'f_position', $_POST['f_position']);
            else {
                BFEC::add('stanowisko zawiera niedozwolone znaki');
                $f++;
            }
        ...
        if (!isset($_POST['f_company']) || empty($_POST['f_company']))
            ...
            if (Valid::company($_POST['f_company']))
                RFD::add('profEditForm', 'f_company', $_POST['f_company']);
            else {
                BFEC::add('w nazwie firmy wystepuja niedozwolone znaki');
                $f++;
            }
        ...
        if (!isset($_POST['f_street']) || empty($_POST['f_street']))
            ...
            if (Valid::street($_POST['f_street']))
                RFD::add('profEditForm', 'f_street', $_POST['f_street']);
            else {
                BFEC::add('w nazwie ulicy wystepuja niedozwolone znaki');
                $f++;
            }
        ...
        if (!isset($_POST['f_house_number']) || empty($_POST['f_house_number']))
            ...
            if (Valid::house_number($_POST['f_house_number']))
                RFD::add('profEditForm', 'f_house_number', $_POST['f_house_number']);
            else {
                BFEC::add('w numerze domu/mieszkania wystepuja niedozwolone znaki');
                $f++;
            }
        ...
        if (!isset($_POST['f_postcode']) || empty($_POST['f_postcode']))
            ...
            if (Valid::postcode($_POST['f_postcode']))
                RFD::add('profEditForm', 'f_postcode', $_POST['f_postcode']);
            else {
                BFEC::add('w kodzie pocztowym wystepuja niedozwolone znaki');
                $f++;
            }
        ...
        if (!isset($_POST['f_city']) || empty($_POST['f_city']))
            ...
            if (Valid::city($_POST['f_city']))
                RFD::add('profEditForm', 'f_city', $_POST['f_city']);
            else {
                BFEC::add('w miescie wystepuja niedozwolone znaki');
                $f++;
            }
        ...
        if (!isset($_POST['f_phone']) || empty($_POST['f_phone']))
            ...
            if (Valid::phone($_POST['f_phone']))
                RFD::add('profEditForm', 'f_phone', $_POST['f_phone']);
            else {
                BFEC::add('telefon w zlym formacie');
                $f++;
            }
        ...
        if (!isset($_POST['f_nip']) || empty($_POST['f_nip']))
            ...
            if (Valid::nip($_POST['f_nip']))
                RFD::add('profEditForm', 'f_nip', $_POST['f_nip']);
            else {
                BFEC::add('nip w zlym formacie');
                $f++;
            }
        ...
        if (BFEC::isError())
            throw new ErrorsInProfileEditForm('');
    ...
    public function getIdForLeftMenu() {
        ...
        if (isset($_GET['id']) && !empty($_GET['id']))
            $id = $_GET['id'];
        else if (isset($_GET['subsubcat']) && !empty($_GET['subsubcat']))
            $id = $_GET['subsubcat'];
        ...
        if (!is_null($id) && !Valid::csid($id))
            $id = null; // sprawdzamy poprawnosc ID
    ...
    public function getCommGroupJoinFormData(Commision $c) {
        ...
        if ($errors)
            throw new SomeErrors();
        if ($parts_count == 0)
            throw new NoMembersToJoin();
        if ($parts_count + $c->getParts_count() > 16)
            throw new TooManyMembers();
    ...
    public function getLoginFormData() {
        ...
        if (!isset($_POST['email']) || empty($_POST['email'])) {
            ...
            RFD::add('logForm', 'email', $_POST['email']);
    ...
    public function getCommision() {
        ...
        //
        if (!isset($_POST['regulamin']) || $_POST['regulamin'] != 1)
            BFEC::add(BFEC::$e['UD']['regulamin']);

        //
        // POLA OBOWIAZKOWE
        //
        if (!isset($_POST['cat']) || empty($_POST['cat']) || !Valid::kategoria($_POST['cat']))
            BFEC::add(BFEC::$e['UD']['kategoria']);
        else
            RFD::add('addCommForm', 'cat', $_POST['cat']);
        ...
        if (!isset($_POST['subcat']) || empty($_POST['subcat']) || !Valid::obszar($_POST['subcat']))
            BFEC::add(BFEC::$e['UD']['obszar']);
        else
            RFD::add('addCommForm', 'subcat', $_POST['subcat']);

        if (!isset($_POST['subsubcat']) || empty($_POST['subsubcat']) || !Valid::tematyka($_POST['subsubcat']))
            BFEC::add(BFEC::$e['UD']['tematyka']);
        else
            RFD::add('addCommForm', 'subsubcat', $_POST['subsubcat']);

        if (!isset($_POST['moduly']) || empty($_POST['moduly']) || !is_array($_POST['moduly']) || count($_POST['moduly']) <= 0)
            BFEC::add(BFEC::$e['UD']['moduly']);
        else {
            $valid = true;
            ...
            if ($valid)
                RFD::add('addCommForm', 'moduly', $_POST['moduly']);
            else
                BFEC::add(BFEC::$e['UD']['moduly']);
        }

        if (!isset($_POST['long']) || empty($_POST['long']) || !Valid::add_comm_long($_POST['long']))
            BFEC::add(BFEC::$e['UD']['long']);
        else
            RFD::add('addCommForm', 'long', $_POST['long']);

        if (!isset($_POST['days']) || empty($_POST['days']) || !is_array($_POST['days']) || !Valid::add_comm_days($_POST['days']))
            BFEC::add(BFEC::$e['UD']['days']);
        else {
            if (!Valid::add_comm_days_continuity($_POST['days'], RFD::get('addCommForm', 'long')))
                BFEC::add(BFEC::$e['UD']['days_continuity']);
            else
                RFD::add('addCommForm', 'days', $_POST['days']);
        }

        if (!isset($_POST['date_a']) || empty($_POST['date_a']) || !Valid::add_comm_date($_POST['date_a']))
            BFEC::add(BFEC::$e['UD']['date']);
        else
            RFD::add('addCommForm', 'date_a', $_POST['date_a']);

        if (!isset($_POST['date_b']) || empty($_POST['date_b']) || !Valid::add_comm_date($_POST['date_b']))
            BFEC::add(BFEC::$e['UD']['date']);
        else
            RFD::add('addCommForm', 'date_b', $_POST['date_b']);

        if (!Valid::add_comm_date_long(UF::date2timestamp(RFD::get('addCommForm', 'date_a')), UF::date2timestamp(RFD::get('addCommForm', 'date_b')), RFD::get('addCommForm', 'long')))
            BFEC::add(BFEC::$e['UD']['date_long']);

        if (isset($_POST['drugi_termin'])) {
            if (!isset($_POST['date_c']) || empty($_POST['date_c']) || !Valid::add_comm_date($_POST['date_c']))
                BFEC::add(BFEC::$e['UD']['date']);
            else
                RFD::add('addCommForm', 'date_c', $_POST['date_c']);

            if (!isset($_POST['date_d']) || empty($_POST['date_d']) || !Valid::add_comm_date($_POST['date_d']))
                BFEC::add(BFEC::$e['UD']['date']);
            else
                RFD::add('addCommForm', 'date_d', $_POST['date_d']);

            if (!Valid::add_comm_date_long(UF::date2timestamp(RFD::get('addCommForm', 'date_c')), UF::date2timestamp(RFD::get('addCommForm', 'date_d')), RFD::get('addCommForm', 'long')))
                BFEC::add(BFEC::$e['UD']['date_long']);
        }

        if (!isset($_POST['expire']) || empty($_POST['expire']) || !Valid::waznosc($_POST['expire']))
            BFEC::add(BFEC::$e['UD']['expire']);
        else
            RFD::add('addCommForm', 'expire', $_POST['expire']);

        if (!isset($_POST['place']) || empty($_POST['place']) || !Valid::city($_POST['place']))
            BFEC::add(BFEC::$e['UD']['place']);
        else
            RFD::add('addCommForm', 'place', $_POST['place']);

        if (isset($_POST['woj']) && !empty($_POST['woj']) && Valid::woj($_POST['woj']))
            RFD::add('addCommForm', 'woj', $_POST['woj']);

        $cena1 = $cena2 = false;

        if (!isset($_POST['cena_min']) || empty($_POST['cena_min']) || !Valid::price($_POST['cena_min']))
            BFEC::add(BFEC::$e['UD']['cena_min']);
        else
            $cena1 = true;

        if (!isset($_POST['cena_max']) || empty($_POST['cena_max']) || !Valid::price($_POST['cena_max']))
            BFEC::add(BFEC::$e['UD']['cena_max']);
        else
            $cena2 = true;
        ...
        for ($i = 2; $i <= 16; $i++) {
            if (isset($_POST['part' . $i . '_name']) && !empty($_POST['part' . $i . '_name']) && isset($_POST['part' . $i . '_surname']) && !empty($_POST['part' . $i . '_surname'])) {
                if (Valid::name($_POST['part' . $i . '_name']) && Valid::name($_POST['part' . $i . '_surname'])) {
                    ...
                    BFEC::add(BFEC::$e['UD']['uczestnik']);
            }
        }

        if ($participants < 1)
            BFEC::add(BFEC::$e['UD']['liczba_ucz']);
        if (count($parts) > 0)
            RFD::add('addCommForm', 'parts', $parts);
        ...
        if (isset($_POST['drugi_termin'])) {
            if (isset($_POST['date_c']))
                $c->setDate_c($_POST['date_c']);
            if (isset($_POST['date_d']))
                $c->setDate_d($_POST['date_d']);
        }
    ...
    public function getService() {
        ...
        // POLA OBOWIAZKOWE
        //
        if (!isset($_POST['name']) || empty($_POST['name']) || !Valid::serv_name($_POST['name']))
            BFEC::add(BFEC::$e['UD']['name']);
        else
            RFD::add('addServForm', 'name', $_POST['name']);

        if (!isset($_POST['cat']) || empty($_POST['cat']) || !Valid::kategoria($_POST['cat']))
            BFEC::add(BFEC::$e['UD']['kategoria']);
        else
            RFD::add('addServForm', 'cat', $_POST['cat']);
        ...
        if (!isset($_POST['subcat']) || empty($_POST['subcat']) || !Valid::obszar($_POST['subcat']))
            ...
            RFD::add('addServForm', 'subcat', $_POST['subcat']);

        if (!isset($_POST['subsubcat']) || empty($_POST['subsubcat']) || !Valid::tematyka($_POST['subsubcat']))
            BFEC::add(BFEC::$e['UD']['tematyka']);
        else
            RFD::add('addServForm', 'subsubcat', $_POST['subsubcat']);
        ...
        if (!isset($_POST['moduly']) || empty($_POST['moduly']) || !is_array($_POST['moduly']) || count($_POST['moduly']) <= 0)
            ...
            if ($valid)
                RFD::add('addServForm', 'moduly', $_POST['moduly']);
            else
                $m = false;
        }
        ...
        if (!isset($_POST['program']) || empty($_POST['program']))
            ...
            if (mb_strlen($_POST['program'], 'UTF-8') > 5000)
                BFEC::add(MSG::addServProgramZaDlugi());

            RFD::add('addServForm', 'program', $_POST['program']);
        }

        if (!$m && !$p)
            BFEC::add(BFEC::$e['UD']['brak_programu']);

        if (!isset($_POST['date_uzg'])) {
            if (!isset($_POST['date_a']) || empty($_POST['date_a']) || !Valid::add_comm_date($_POST['date_a']))
                BFEC::add(BFEC::$e['UD']['date']);
            else
                RFD::add('addServForm', 'date_a', $_POST['date_a']);

            if (!isset($_POST['date_b']) || empty($_POST['date_b']) || !Valid::add_comm_date($_POST['date_b']))
                BFEC::add(BFEC::$e['UD']['date']);
            else
                RFD::add('addServForm', 'date_b', $_POST['date_b']);
        }
        else
            RFD::add('addServForm', 'date_uzg', true);

        if (!isset($_POST['place']) || empty($_POST['place']) || !Valid::city($_POST['place']))
            BFEC::add(BFEC::$e['UD']['place']);
        else
            RFD::add('addServForm', 'place', $_POST['place']);

        if (isset($_POST['woj']) && !empty($_POST['woj']) && Valid::woj($_POST['woj']))
            RFD::add('addServForm', 'woj', $_POST['woj']);

        if (!isset($_POST['cena']) || empty($_POST['cena']) || !Valid::price($_POST['cena']))
            BFEC::add(BFEC::$e['UD']['cena']);
        else
            RFD::add('addServForm', 'cena', $_POST['cena']);

        if (!isset($_POST['cena_']) || empty($_POST['cena_']) || !Valid::add_serv_cena_($_POST['cena_']))
            BFEC::add(BFEC::$e['UD']['cena_']);
        else
            RFD::add('addServForm', 'cena_', $_POST['cena_']);
        ...
        if (!isset($_POST['mail']) || empty($_POST['mail']) || !Valid::email($_POST['mail']))
            BFEC::add(BFEC::$e['UD']['NoValidEmail']);
        else
            RFD::add('addServForm', 'mail', $_POST['mail']);

        if (!isset($_POST['phone']) || empty($_POST['phone']) || !Valid::phone($_POST['phone']))
            BFEC::add(BFEC::$e['UD']['phone']);
        else
            RFD::add('addServForm', 'phone', $_POST['phone']);
        ...
        if (isset($_POST['contact']) && !empty($_POST['contact']) && Valid::contact($_POST['contact']))
            RFD::add('addServForm', 'contact', $_POST['contact']);
        ...
        if (BFEC::isError())
            throw new ErrorsInAddServForm();

        $s->setName($_POST['name']);
        $s->setCat($_POST['cat']);
        $s->setSubcat($_POST['subcat']);
        $s->setSubsubcat($_POST['subsubcat']);
        if (!is_null(RFD::get('addServForm', 'moduly')))
            $s->setModuly(RFD::get('addServForm', 'moduly'));
        if (!is_null(RFD::get('addServForm', 'program')))
            $s->setProgram(RFD::get('addServForm', 'program'));
        if (!is_null(RFD::get('addServForm', 'date_uzg')))
            $s->setDate_uzg(RFD::get('addServForm', 'date_uzg'));
        if (!is_null(RFD::get('addServForm', 'date_a')))
            $s->setDate_a(RFD::get('addServForm', 'date_a'));
        if (!is_null(RFD::get('addServForm', 'date_b')))
            $s->setDate_b(RFD::get('addServForm', 'date_b'));
        $s->setPlace($_POST['place']);
        if (!is_null(RFD::get('addServForm', 'woj')))
            $s->setWoj(RFD::get('addServForm', 'woj'));
        $s->setCena($_POST['cena']);
        $s->setCena_($_POST['cena_']);
        $s->setMail($_POST['mail']);
        $s->setPhone($_POST['phone']);
        if (!is_null(RFD::get('addServForm', 'contact')))
            $s->setContact(RFD::get('addServForm', 'contact'));
        if (!is_null(RFD::get('addServForm', 'desc')))
            $s->setDesc(RFD::get('addServForm', 'desc'));
        $s->setDate_add(time());
    ...
    public function getIDFromURL() {
        if (!is_numeric($_GET['id']))
            throw new InvalidID('Próba wywołania zlecenia z niepoprawnym ID: ' . $_GET['id']);
        $id = intval($_GET['id']);
        if ($id <= 0)
            throw new InvalidID('Próba wywołania zlecenia z niepoprawnym ID: ' . $_GET['id']);
    ...
    public function getCommOffer() {
        if (!isset($_GET['id']) || !Valid::isNatural($_GET['id']))
            BFEC::add('Niepoprawny ID!');

        if (isset($_GET['cena']) && Valid::price($_GET['cena']))
            RFD::add('CO', 'cena', $_GET['cena']);
        else
            BFEC::add(BFEC::$e['UD']['cena']);

        if (!isset($_GET['cenax']) || $_GET['cenax'] < 1 || $_GET['cenax'] > 3)
            BFEC::add(BFEC::$e['UD']['cena_']);
        else
            RFD::add('CO', 'cenax', $_GET['cenax']);

        if (!isset($_GET['rozl']) || $_GET['rozl'] < 1 || $_GET['rozl'] > 3)
            BFEC::add('Musisz podać sposób rozliczania!');
        else
            RFD::add('CO', 'rozl', $_GET['rozl']);
        ...
        if (isset($_GET['inne']) && is_array($_GET['inne'])) {
            ...
            if ($inne[strlen($inne) - 1] == '4') {
                if (isset($_GET['ile_kaw']) && Valid::isNatural($_GET['ile_kaw']))
                    RFD::add('CO', 'ile_kaw', $_GET['ile_kaw']);
                else
                    BFEC::add('Musisz podać ilość przerw kawowych!');
            }
        }

        if (isset($_GET['date_a']) && Valid::add_comm_date($_GET['date_a']) !== false)
            RFD::add('CO', 'date_a', $_GET['date_a']);
        else
            BFEC::add(BFEC::$e['UD']['date']);

        if (isset($_GET['date_b']) && Valid::add_comm_date($_GET['date_b']) !== false)
            RFD::add('CO', 'date_b', $_GET['date_b']);
        else
            BFEC::add(BFEC::$e['UD']['date']);
        ...
        $o->setRozl($_GET['rozl']);
        if (isset($_GET['inne']) && !empty($_GET['inne']))
            $o->setInne($_GET['inne']);
        if (isset($_GET['ile_kaw']) && !empty($_GET['ile_kaw']))
            $o->setIle_kaw($_GET['ile_kaw']);
